...
export function parseAnimatedValue(
    ...
    if (full?.groups) {
        const baseline = defaultFrom ?? 0;
        return {
            from: full.groups.from ? Number.parseFloat(full.groups.from) : baseline,
            to: full.groups.to ? Number.parseFloat(full.groups.to) : baseline,
